...
public class EmpresaTaxis {
	...
	public EmpresaTaxis(){
		...
		Taxi taxiA = new Taxi ("GPR-396", 2012);
		Taxi taxiB = new Taxi ("LSQ-713",2010);
		
		
		taxiA.agregarConductor("Phoebe", Conductor.TURNO_MEDIO);
		taxiB.agregarConductor("Joey" , Conductor.TURNO_CORTO);
		taxiB.agregarConductor("Chandler", Conductor.TURNO_LARGO);
	...
	public double calcularGastosTotales(){
		...
		double gastosTotales = taxiA.calcularGastosSemanales() + taxiB.calcularGastosSemanales();
	...
	public double calcularBalanceTotal(){
		...
		// ingresos son equivalentes a (entrega turno respestivo + ingresos) - gastossemanales
		
		balance = calcularGastosTotales() - calcularIngresosTotales();
