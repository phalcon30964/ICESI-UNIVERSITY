...
public class GrafoLista<T> implements IGrafo<T>, Serializable{
	...
	public ListaDoble<Arista<T>>[] aristas;
	...
	public String determinarCircuitoEuleriano(GrafoLista<T> g){
		
		String camino = "";
		TreeSet<Arista<T>> ordenar = new TreeSet<Arista<T>>();
		Pila<Arista<T>> arist = new Pila<Arista<T>>();
		
		T actual = g.vertices[0];
		int posActual = 0;
		
		for (int i = 0; i < g.aristas[posActual].darLongitud(); i++) {
			ordenar.add((g.aristas[posActual].darElemento(i)));
		}
