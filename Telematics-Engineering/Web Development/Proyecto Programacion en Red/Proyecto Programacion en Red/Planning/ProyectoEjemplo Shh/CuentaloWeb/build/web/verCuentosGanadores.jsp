            ...
            <div id="mid_content">
                <div class="container">
                    <div class="mid_content_info mid_content_space">
                        ...
                        <DIV ALIGN=center>
                            <form action="leerCuento.jsp" method="POST" style="text-align: center;">
                                <select name="campoCuentos" style="text-align: center;">

                                    <%
                                        HttpSession sesion = request.getSession();

                                        String elConcurso = "";
                                        if (request.getParameter("campoCerrados") != null) {
                                            elConcurso = request.getParameter("campoCerrados");
                                            sesion.setAttribute("campoCerrados", elConcurso);
                                        } else {
                                            elConcurso = (String) sesion.getAttribute("campoCerrados");
                                        }

                                        if ((sesion.getAttribute("tipoUsuario") != null) || !(elConcurso.equalsIgnoreCase(""))) {

                                            String[] infoConcurso = elConcurso.split("-");
                                            int idConcurso = Integer.parseInt(infoConcurso[0]);
                                            ArrayList<String> cuentosConcurso = AdminBD.darCuentos(idConcurso);
                                            String elCuento = "";
                                            for (int i = 0; i < cuentosConcurso.size(); i++) {
                                                elCuento = cuentosConcurso.get(i);
                                    %>
                                    <option value="<%=elCuento%>"><%=elCuento%></option>
                                    <%}%>
                                </select>
                                <br>
                                <input name="btnVerCuentosParticipantes" type="submit" value="Leer Cuento" />
                            </form>
                        </DIV>
                    </div>
                    <div id="footer">
